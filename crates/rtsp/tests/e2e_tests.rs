...
fn test_source_video_formats() {
    ...
    let parsed = waycast_rtsp::parse_sink_video_formats(&video_formats)
...
fn test_codec_negotiation_h264() {
    ...
    assert_eq!(codec, waycast_rtsp::NegotiatedCodec::H264);
}
...
fn test_codec_negotiation_h265() {
    ...
    assert!(
        codec == waycast_rtsp::NegotiatedCodec::H265
            || codec == waycast_rtsp::NegotiatedCodec::H264
    );
}
...
/// Test WFD IEs generated by waycast-net
#[test]
fn test_waycast_wfd_ies_match_spec() {
    let wfd_ies = vec![
        0x00, // Subelement ID: WFD Device Information
        0x00, 0x06, // Length: 6 bytes
        0x00, 0x90, // Device Info: GNOME-compatible source bytes
