...
mod tests {
    ...
    fn test_cli_parsing() {
        ...
        let cmd = Cli::try_parse_from(&["waycast", "discover", "-t", "20"]);
        assert!(cmd.is_ok());
        match cmd.unwrap().command {
    ...
    #[test]
    fn test_json_flag() {
        let cmd = Cli::try_parse_from(&["waycast", "--json", "doctor"]);
        assert!(cmd.is_ok());
    ...
    fn test_invalid_command() {
        let cmd = Cli::try_parse_from(&["waycast", "invalid"]);
        assert!(cmd.is_err());
    }

    #[test]
    fn test_help_command() {
        let result = std::panic::catch_unwind(|| Cli::try_parse_from(&["waycast", "--help"]));
